...
export function Game() {
  ...
  return (
    <div className="flex flex-col w-full max-w-lg mx-auto bg-stone-50 min-h-[100dvh] relative">
      ...
      {/* Header */}
      <header className="flex items-center justify-between p-4 bg-white shadow-sm z-10 relative">
        <button onClick={goHome} className="p-2 -ml-2 text-stone-500 hover:text-stone-900 rounded-full hover:bg-stone-100 transition-colors">
          <ArrowLeft size={24} />
        </button>
        <div className="flex flex-col items-center flex-1">
          <h2 className="text-xl font-bold text-stone-900">{puzzle.ambientName}</h2>
        ...
        <div className="flex items-center gap-1 -mr-2">
          <button 
            ...
            className={`p-2 rounded-full transition-colors ${history.length === 0 ? 'text-stone-300 cursor-not-allowed' : 'text-stone-500 hover:text-stone-900 hover:bg-stone-100'}`}
          >
            <Undo2 size={20} />
          </button>
          <button onClick={() => useGameStore.getState().clearBoard()} className="p-2 text-stone-500 hover:text-stone-900 rounded-full hover:bg-stone-100 transition-colors">
            <RotateCcw size={20} />
          </button>
        </div>
      </header>
      ...
      <AnimatePresence>
        {currentView === 'won' && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="absolute inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-6 z-50"
          >
            <motion.div 
